...
st.latex(r'''
        r(φ) = [sin(\frac{n_1 φ}{2}) + 0.2 * sin(\frac{3 n_1 φ}{2})] ^{n_2}
        ''')

col1, col2 = st.columns(2)
with col1:
    n1 = st.slider("n1", min_value=1, max_value=6, value=3,  step=1)
with col2:    
    n2 = st.slider("n2", min_value=2, max_value=6, value=2,  step=2)
txt1 = ''    
if n1==3:
...
rnd = 0.05*np.random.rand(theta.size)
...
# r = (np.sin(n1*theta/2) + 0.2 * np.sin(n1*4.5*theta/3))**n2 + 0.05*np.sin(100*n1*theta/2)

r = (np.sin(n1*theta/2) + 0.2 * np.sin(n1*4.5*theta/3))**n2 
r1 = 0.7 * r
r2 = 0.05 * r
